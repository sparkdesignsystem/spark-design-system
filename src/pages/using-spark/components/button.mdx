...
> As a general rule, use one main call-to-action button (Primary button) per screen but no more than two.
...
If the action will take take the user to a new page or to an external site, consider using a [Link](/using-spark/components/link) instead.

### Guidelines

#### How to Write Effective Button Text
- Button text should set clear expectation of what will happen on click.
...
- Use first-person pronouns. Use “Customize My Loan,”  “Customize Your Loan.”
...
- Width should be more than 48px, but less than 280px.
- Buttons should have at least 24px of space away from any surrounding elements.
...
The main call to action on the page. Use no more than two Primary Buttons per page.

> When there are multiple actions, use the primary button to indicate the main option.
...
### Tertiary Button
Represents less prominent actions. Sometimes in in an action set with [Primary](#primary-button) and [Secondary](#secondary-button) Buttons, but they can also be independent actions.
