...
it ('should render a button cta with the correct properties', () => {
  const wrapper = mount( <SprkPromo cta='button' ctaHref='foo' ctaAnalytics='bar' ctaIdString='baz' ctaText='foobar'/>);
...
const wrapper = mount( <SprkPromo cta='link' ctaHref='foo' ctaAnalytics='bar' ctaIdString='baz' ctaText='foobar'/>);
...
it ('should render a flag image with passed in properties in the reversed variant', () => {
  const wrapper = shallow(<SprkPromo isFlag imgAlt='foo' imgSrc='bar' mediaRev />);
  const img = wrapper.find('img.sprk-c-Promo__image--flag');
