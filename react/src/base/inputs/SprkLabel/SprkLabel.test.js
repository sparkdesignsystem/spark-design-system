...
describe('SprkLabel:', () => {
  ...
  it('should add forwardedRef to the label', () => {
    // Create ref
    const forwardedRef = React.createRef();
    // Add ref as prop
    const wrapper = mount(<SprkLabel forwardedRef={forwardedRef} />);
    ...
    expect(
      wrapper.find('.sprk-b-Label').getDOMNode().getAttribute('data-test'),
    ).toBe('testing');
  });
});
