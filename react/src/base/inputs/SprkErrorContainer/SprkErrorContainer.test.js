...
it('SprkErrorContainer: Should render an element with the correct class', () => {
  const wrapper = mount(<SprkErrorContainer id={id} message={message} />);
  expect(wrapper.find('.sprk-b-ErrorContainer').length).toBe(1);
});

it('SprkErrorContainer: Should add classes when additionalClasses has a value', () => {
  const wrapper = mount(
...
it('SprkErrorContainer: Should assign data-analytics when analyticsString has a value', () => {
  const wrapper = mount(<SprkErrorContainer id={id} message={message} analyticsString="321" />);
  expect(wrapper.find('[data-analytics="321"]').length).toBe(1);
});

it('SprkErrorContainer: Should assign data-id when idString has a value', () => {
