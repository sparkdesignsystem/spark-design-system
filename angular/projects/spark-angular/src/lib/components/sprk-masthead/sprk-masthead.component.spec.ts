...
describe('SprkMastheadComponent', () => {
  ...
  it('should set the logo link and text to provided values using href input', () => {
    component.logoHref = '/alert';
    component.logoLinkScreenReaderText = 'Go to the alerts page.';
    fixture.detectChanges();
    const logoEl = mastheadElement.querySelector('.sprk-c-Masthead__branding');
    ...
    expect(href).toEqual('/alert');
    expect(logoElement.textContent.trim()).toEqual('Go to the alerts page.');
  });

  it('should set the logo routerLink', () => {
    component.logoRouterLink = '/router-link-test';
    component.logoLinkScreenReaderText = 'Router Link Test';
    fixture.detectChanges();
    const logoElement: HTMLElement = mastheadElement.querySelector(
      '.sprk-c-Masthead__branding a',
    );
    expect(logoElement.getAttribute('href')).toEqual('/router-link-test');
    expect(logoElement.textContent.trim()).toEqual('Router Link Test');
  });
